...
module.exports = async ({ config, hooks, listeners }) => {
  ...
  Object.assign(app.aedes, hooks(app))

  Object.entries(listeners(app)).forEach((entry) => app.aedes.on(...entry))

  app.wsServer = ws.createServer({ server: httpServer }, app.aedes.handle)

  httpServer.listen(config.ws.port, () => {
    app.logger.info(`Websocket server listening on port ${config.ws.port}`)
  })

  app.tlsServer = tls.createServer(tlsOptions, app.aedes.handle)
