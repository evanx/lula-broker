...
module.exports = (app) => async (client, username, password, callback) => {
  ...
  if (username !== client.id) {
    logger.warn(
      `Mismatched username "${username}" for client "${client.id}"`,
    )
  } else if (password) {
    username = username.replace(/:/g, '')
    const bcryptPassword = await redisClient.hget(
      `broker:client:${client.id}:h`,
      'password',
    )
    if (bcryptPassword) {
    ...
    const registerDeadline = await redisClient.zscore(
      'broker:register:z',
      username,
    )
    if (registerDeadline) {
      await redisClient.zrem('broker:register:z', username)
